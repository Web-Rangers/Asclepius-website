    ...
    for (let i = 1; i <= daysCount; i++) {
      ...
      days.push(
        <Day
          ...
          onClick={() => {
            setSelected(new Date(date.getFullYear(), date.getMonth(), i));
          }}
          number={i}
          style={{
...
const RangePicker = ({ isOpen, onClose, dateStartChange, dateEndChange }) => {
  const [days, setDays] = useState([]);
        ...
        <Day
          ...
          className={classNames({
            ...
            [styles.disabled]:
              target === "end" &&
              dateStart &&
              new Date(date.getFullYear(), date.getMonth(), i) < dateStart,
            [styles.inRange]:
              dateStart &&
              dateEnd &&
              new Date(date.getFullYear(), date.getMonth(), i) > dateStart &&
              new Date(date.getFullYear(), date.getMonth(), i) < dateEnd,
          })}
          onClick={() => {
            if (target === "start") {
              setTarget("end");
            ...
            setTarget("start");
            setDateEnd(new Date(date.getFullYear(), date.getMonth(), i));
          }}
          number={i}
          style={{
            gridColumnStart: column,
  ...
  function configureNextDays() {
    const daysCount = daysInMonth(nextDate.getMonth(), nextDate.getFullYear());
    const days = [];
    for (let i = 6; i < 13; i++) {
      days.push(
        <DayHeader
      ...
      days.push(
        <Day
          ...
          className={classNames({
            [styles.selected]:
              (dateStart &&
                nextDate.getFullYear() === dateStart.getFullYear() &&
                nextDate.getMonth() === dateStart.getMonth() &&
                dateStart.getDate() === i) ||
              (dateEnd &&
                nextDate.getFullYear() === dateEnd.getFullYear() &&
                nextDate.getMonth() === dateEnd.getMonth() &&
                dateEnd.getDate() === i),
            [styles.disabled]:
              target === "end" &&
              dateStart &&
              new Date(nextDate.getFullYear(), nextDate.getMonth(), i) <
                dateStart,
            [styles.inRange]:
              dateStart &&
              dateEnd &&
    ...
    const nextDaysCount =
      7 - getDayOfWeek(daysCount, nextDate.getMonth(), nextDate.getFullYear());
    for (let i = 1; i <= nextDaysCount; i++) {
      const column = getDayOfWeek(
        i,
        nextDate.getMonth() + 1,
