...
Date.prototype.GetLastDayOfWeek = function () {
	const date = new Date(this.getFullYear(), this.getMonth(), this.getDate());
	return new Date(
		date.setDate(date.getDate() - date.getDay() + (date.getDay() == 0 ? 0 : 7))
	);
};
...
export default function Calendar({ booking, workingDaysData }) {
	...
	return (
		<div className={styles.container}>
			{whole ? (
				<div className={styles.calendar}>
					<div className={styles.calendarHeader}>
						<div className={styles.datePeriod}>
							...
							{mode === 'week' &&
								`${date.GetFirstDayOfWeek().getDate()} - ${date
									.GetLastDayOfWeek()
									.getDate()} ${
									MonthName[date.GetLastDayOfWeek().getMonth()]
								}, ${date.GetLastDayOfWeek().getFullYear()}`}
							{mode === 'day' &&
								`${date.getDate()} ${
							...
							<span
								onClick={() => {
									...
									if (mode === 'week' && prev !== 0) {
										...
										setDate(
											new Date(
												date.GetFirstDayOfWeek().getFullYear(),
												date.GetFirstDayOfWeek().getMonth(),
												date.GetFirstDayOfWeek().getDate() - 7
											)
										);
									}
									if (mode === 'day') {
								...
								onClick={() => {
									...
									if (mode === 'week' && next !== 2) {
										...
										setDate(
											new Date(
												date.GetLastDayOfWeek().getFullYear(),
												date.GetLastDayOfWeek().getMonth(),
												date.GetLastDayOfWeek().getDate() + 7
											)
										);
									}
									if (mode === 'day') {
				...
				<div className={styles.calendar}>
					<div className={styles.calendarHeader}>
						<div className={styles.datePeriod}>17-20 June, 2022</div>
						<div className={styles.calendarControls}>
							<ReactSVG
					...
					<div className={styles.calendarBody}>
						{Array.from({ length: 4 }).map((_, i) => (
							<div
								key={i}
								className={classNames(styles.smallDay, {
									[styles.today]: i === 0,
								})}
							>
								<div className={styles.dayHeader}>
									<div className={styles.dayString}>{DayName[i]}</div>
									<div className={styles.dayDate}>{17 + i}</div>
								</div>
...
function MonthView({ date, schedule = [] }) {
	...
	function configureDays() {
		...
		for (let i = 0; i < 7; i++) {
			days.push(
				<DayHeader
					title={DayName[i]}
					className={i === 0 ? styles.left : i === 6 ? styles.right : null}
				/>
			);
		}
		let row = 2;
	...
	function configureEvents() {
		const events = schedule
			.filter((day) => day.date.getMonth() === date.getMonth())
			.map(({ date, events }) => {
				return (
					<div
					...
					>
						...
						{events.length > 2 && (
							<div className={styles.moreEvents}>
								<ReactSVG
									src='/images/icons/calendar/more.svg'
									className={styles.iconContainer}
								/>
							</div>
						)}
...
function WeekView({
	...
	function configureEvents() {
		const events = schedule.map((day) => {
			...
			if (
				dayDate.getTime() >= date.GetFirstDayOfWeek().getTime() &&
				dayDate.getTime() <= date.GetLastDayOfWeek().getTime()
			) {
					...
					return (
						<Event
							...
							style={{
								...
								gridColumnEnd: column + 1,
								gridRowStart:
									parseInt(hourMinutes[0]) * 60 + parseInt(hourMinutes[1]) > 0
										? parseInt(hourMinutes[0]) * 60 + parseInt(hourMinutes[1])
										: 1,
								gridRowEnd:
									parseInt(hourMinutes[0]) * 60 + parseInt(hourMinutes[1]) + 42,
								backgroundColor: opacityColor(event.color, 0.05),
								borderLeft: `3px solid ${event.color}`,
								color: event.color,
								height: 'auto',
					...
					{Array.from({ length: 7 }).map((_, i) => (
						...
						>
							<DayHeader
								title={DayName[i]}
								className={
									i === 0 ? styles.left : i === 7 ? styles.right : null
								}
							/>
							<span
								className={classNames(styles.date, {
									[styles.activeDay]: active === DayName[i],
								})}
								onClick={() => {
									setActive(DayName[i]);
									chooseDay(date.GetWeekDay(i + 1));
									setAvailableTimes(workingDaysData);
								}}
							>
								{date.GetWeekDay(i).getDate()}
							</span>
						</span>
					))}
				</div>
...
function Hour({ number, ...props }) {
	return (
		<div
			...
			{...props}
		>
			<span className={styles.hourNumber}>{`${
