...
const ClinicDetailPage = () => {
	...
	useEffect(() => {
		...
		const categorie = categories?.filter(
			(e) => e.id == router?.query?.categoryId
		)[0];
		const parent = categories?.filter(
			(e) => e.id == categorie?.parentCategoryId
		)[0];

		getData(`${process.env.MEDICAL_API}/medical/clinics/${router?.query?.id}`)
			.then((response) => {
				const phone = response?.contactInfos?.filter(
					(e) => e.type.value === 'mobile'
				);
					...
					<div className={styles.breadcrumbs}>
						<Breadcrumb separator={<img src='/separator.svg' />}>
							...
							{generateBreadcrumbs?.parent && (
								<Breadcrumb.Item>
									<Link
										href={`/clinics?categoryId=${generateBreadcrumbs.parent.id}`}
									>
										<span className={styles.breadcrumbSpan}>
											{generateBreadcrumbs.parent.title}
										</span>
									</Link>
								</Breadcrumb.Item>
							)}
							<Breadcrumb.Item>
								{generateBreadcrumbs?.parent ? (
									<Link
										href={`/clinics/?categoryId=${generateBreadcrumbs?.categorie?.id}&parentCategory=${generateBreadcrumbs?.parent?.id}`}
									>
										<span className={styles.breadcrumbSpan}>
											{generateBreadcrumbs?.categorie?.title}
										</span>
									</Link>
								) : (
									generateBreadcrumbs?.categorie && (
										<Link
											href={`/clinics/?categoryId=${generateBreadcrumbs?.categorie?.id}`}
										>
											<span className={styles.breadcrumbSpan}>
												{generateBreadcrumbs?.categorie?.title}
											</span>
								...
								<Link
									href={
										router?.query?.categoryId
											? `/clinicDetailPage/${clinic?.id}/doctors?${doctorsLink}`
											: `/clinicDetailPage/${clinic?.id}/doctors`
									}
								>
					...
					{branches?.map((item, i) => (
						<SwiperSlide key={i}>
							<div className={styles.branchView}>
								...
								<div className={styles.branchWorkingHours}>სამუშაო საათები</div>
