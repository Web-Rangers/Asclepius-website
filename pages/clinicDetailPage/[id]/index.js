    ...
    <div className={s.container}>
      ...
      <div className={s.clinicDetailPageCard}>
        <div className={s.cardItemContainer} key={cardData?.id}>
          ...
          {cardData?.contactInfos && cardData?.contactInfos?.map((item, index) => (
...
export const getServerSideProps = async (ctx) => {
  ...
  const getClinicById = await getData(`https://asclepius.pirveli.ge/asclepius/v1/api/clinics/${userId}`)
  const getClinicAddress = await getData(`https://asclepius.pirveli.ge/asclepius/v1/api/clinics/${userId}/address`)
  const getClinicBranches = await getData(`https://asclepius.pirveli.ge/asclepius/v1/api/clinics/${userId}/branches`)
