...
export default function UserDetailed() {
	...
	useEffect(() => {
		getData(
			`${process.env.MEDICAL_API}/medical/products/get-bought-products`
		).then((response) => {
			const data = response?.products?.map((e) => {
				return {
					...
					productType:
						e.productType === 'PERCENTAGE_CLINIC_DISCOUNT_FAMILY'
							? 'Family'
							: 'Individual',
				};
			});
			...
			var myDate = new Date();
			var day =
				24 * 60 * 60 * 1000 * response?.products[0]?.endDateIncrementValues;
				...
				<div className={styles.detailPageContainer}>
					...
					>
						<div className={styles.greeting}>
							<div>
								<h2>
									გამარჯობა, <span>{userInfo?.firstName}</span>
								</h2>
								<p>
									წარმატებულ დღეს გისურვებთ და არ დაგავიწყდეთ თქვენს
					...
					<div className={styles.rightMenu}>
						<Block className={styles.userblock}>
							{user ? (
								<div className={styles.userBlcokItem}>
									<div
									...
									<Link href='https://profile.pirveli.com'>
										<ReactSVG
											className={styles.userOptionBtn}
											src='/useroption.svg'
										/>
									</Link>
								</div>
						...
						{userCards?.length > 1 ? (
								...
								<div className={styles.cardBlockDiv}>
									<div className={styles.cardView}>
										...
										<span className={styles.cardNumber}>
											2017 1115 2020 {randomNum}
										</span>
										<span className={styles.cardUserTitle}>
											{userInfo?.firstName} {userInfo?.lastName}
										</span>
							...
							>
								<span className={styles.cardsSpan}>
									შეუკვეთე ბარათი და მიიღე 20%-მდე ფასდაკლება ნებისმიერ
									კლინიკაში, ან ექიმთან ვიზიტის დროს
									<button className={styles.buycardForUser}>
										შეუკვეთე ბარათი
									</button>
								</span>
								<Button
			...
			{useWindowSize().width < 600 && (
				<div className={styles.familyMembersAddblock}>
					<div className={styles.familyMembersAddBtn}>
						...
						<ReactSVG
							className={styles.familyMemberSvg}
							src='/familyadd.svg'
						/>
					</div>
				</div>
