        ...
        <div className={styles.detailedPage}>
            <div className={styles.detailPageContainer}>
                <div className={styles.services_container}>
                    ...
                    {
                        services?.map((item)=> {  
                            return <div className={styles.servicesItem} key={item.id}>
                                <div className={styles.serviceItemImg}>
                                    <Image layout='fill' src="/clinicpost.png" />
                                    <div>-{item.discount}</div>
                                </div>
                                <div className={styles.serviceItemInfo}>
                                    <h2>{item.clinicName}</h2>
                                    <button className={styles.clinicWorkingHours}>{item.clinicWorkingHours}</button>
                                    <span>
                                        ...
                                        <div className={styles.servicesList}>
                                            {
                                                item.services?.map((service, i)=> {
                                                    if(i < item.num){
                                                        return <div><span>{service.serviceName}</span> {service.discount}</div>
                                                    }
                                                })
                                            }
                                        </div>
                                    </div>
                                    <button className={classNames(styles.serviceQuantity, {
                                        [styles.showLess]: item.num !== 6
                                    })} onClick={()=> changeServicesquantity(item)}>{item.num == 6 ? 'იხილე მეტი' : 'იხილე ნაკლები'}</button>
                                </div>
                            </div>
                        })
                    }
